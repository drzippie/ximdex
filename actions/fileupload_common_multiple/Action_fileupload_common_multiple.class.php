...
class Action_fileupload_common_multiple extends ActionAbstract
{
    ...
    {
        $source = 'ÀÁÂÃÄÅÆÇÈÉÊËÌÍÎÏÐÑÒÓÔÕÖØÙÚÛÜÝÞ
ßàáâãäåæçèéêëìíîïðñòóôõöøùúûýýþÿŔŕ';
        $target = 'aaaaaaaceeeeiiiidnoooooouuuuy
bsaaaaaaaceeeeiiiidnoooooouuuyybyRr';
        $decodedName = utf8_decode($name);
        $decodedName = strtr($decodedName, utf8_decode($source), $target);
        //$decodedName = strtolower($cadena);
            ...
            if ($estimatedNode > 0) {
                ...
                if ($node->nodeType->get("Name") == "XmlRootFolder") {
                    ...
                    if ($docId > 0) {
                        ...
                        $newDocumentNode->mimetype = $file['type'];
                    ...
                    $data = array(
                        ...
                        'PARENTID' => $idNode,
                        'CHILDRENS' => array(
                            array('NODETYPENAME' => 'PATH', 'SRC' => $file["tmp_name"], 'MIMETYPE' => FsUtils::get_mime_type($file['tmp_name']))
                            //array('NODETYPENAME' => 'PATH', 'SRC' => $file["tmp_name"], 'MIMETYPE' => $file['type'])
                        )
                    );
                    
                    $r = FsUtils::get_mime_type($file['tmp_name']);
                    $baseIO = new baseIO();
                    $result = $baseIO->build($data);
                }
            }
