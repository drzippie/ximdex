...
class Batch extends Batchs_ORM {
	...
	*/
    ...
    function calcCycles($majorCycle,$minorCycle) {
		if(is_null($majorCycle) || is_null($minorCycle)){
	    	$this->BatchToLog(null, null, null, null, null, __CLASS__, __FUNCTION__, __FILE__,
	    			__LINE__, "ERROR", 8, _("ERROR Params $majorCycle - $minorCycle"));
			return null;
		}
		...
		if ($minorCycle > $majorCycle) {
			...
			$minorCycle = 0;
	    	$this->BatchToLog(null, null, null, null, null, __CLASS__, __FUNCTION__, __FILE__,
	    			__LINE__, "INFO", 8, _("Up to major cycle $majorCycle batch ").$this->get('IdBatch')."");
		} else {
			$minorCycle++;
		}
	...
	*/
    ...
    function BatchToLog($batchId, $nodeFrameId, $channelFrameId, $serverFrameId, $pumperId,
	...
	*/
    ...
    function setBatchPlayingOrUnplaying($idBatch, $playingValue = 1) {
		...
		if(!$dbObj->numErr) {
	    	...
	    	$this->BatchToLog($idBatch, null, null, null, null, __CLASS__, __FUNCTION__, __FILE__,
	    			__LINE__, "INFO", 8, _("Setting playing Value = $playingValue for batch $idBatch"));
			return true;
		} else {
	...
	*/
    ...
    function prioritizeBatch($idBatch, $mode = 'up') {
		...
		$dbObj = new DB();
		$time = time();
		$query = "SELECT " . (($mode == 'up') ? "max" : "min") ."(Priority) as priority from Batchs";
		$dbObj->Query($query);

		if (!$dbObj->EOF) {

			$priority = $dbObj->GetValue("priority");
			if ($mode == 'up') {
		...
		if(!$dbObj->numErr) {
	    	...
	    	$this->BatchToLog($idBatch, null, null, null, null, __CLASS__, __FUNCTION__, __FILE__,
	    			__LINE__, "INFO", 8, _("Setting priority Value = $playingValue for batch $idBatch"));
			return true;
		} else {
