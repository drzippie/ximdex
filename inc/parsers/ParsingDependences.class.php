...
class ParsingDependences {
	...
	*/
	...
	/*
    		Function which replace a data array in the xml for a pvd, returning the document content
	*/
	...
	*/
	public static function getAll($idNode, $content, $idVersion) {
		...
		//schema dependency
		$schemaDeps = self::setFormatArray((array) $strDoc->get('IdTemplate'), 'schema');
		...
		//XSL templates dependencies
		$templateDeps = dependencies::getXslDependencies($content, $idNode);
		$templateDeps = sizeof($templateDeps) > 0 ? self::setFormatArray($templateDeps, 'xsltemplate') : array();
		...
		$totalDependencies = array_merge($links, $assets, $ximlets, $symlinks, $schemaDeps, $channelDeps, $languageDeps,$templateDeps, $dotDots, $sectionXimlets, $pathTos, $structure);
	...
	function parseCssDependencies($idNode, $content = NULL) {
		...
		$dependencies = new dependencies();
		//$dependencies->DeleteDependenciesDependentNode($idNode,$version);
		$dependencies->deleteMasterNodeandType($idNode,"LINK");
