		...
		while (list($key, $descriptor) = each($this->_metaData)) {
			...
			switch ($fieldType) {
				...
				case in_array($fieldType, $dataTypeDouble):
					if (!is_numeric($value)) $this->messages->add(sprintf(_("El campo %s tiene un formato inv�lido"), $key), MSG_TYPE_ERROR);
					break;
				...
				case in_array($fieldType, $dataTypeInt):
					if (!is_int((int)$value)) $this->messages->add(sprintf(_("El campo %s tiene un formato inv�lido"), $key), MSG_TYPE_ERROR);
					switch ($fieldType) {
						case 'tinyint':
